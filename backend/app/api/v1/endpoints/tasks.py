...
async def update_task(task_id: str, req: TaskUpdate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    task = await _get_or_404(task_id, current_user.id, db)
...
async def complete_task(task_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    task = await _get_or_404(task_id, current_user.id, db)
...
async def defer_task(task_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    task = await _get_or_404(task_id, current_user.id, db)
...
def _task_dict(t: Task) -> dict:
    return {
        ...
        "status": t.status, "source": t.source, "goal_id": t.goal_id,
        "project_id": t.project_id, "energy_type": t.energy_type,
        "estimated_minutes": t.estimated_minutes, "priority_score": t.priority_score,
        "scheduled_at": t.scheduled_at, "deferral_count": t.deferral_count,
        "completed_at": t.completed_at, "deadline": t.deadline,
