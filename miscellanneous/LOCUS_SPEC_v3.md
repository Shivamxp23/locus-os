...
| `/status` verified from iPhone and Samsung | ⬜ Pending — test manually |
...
- [ ] PWA configuration (manifest, Service Worker via vite-plugin-pwa)
- [ ] Authentication screens (login, register, Genesis Interview)
- [ ] Dashboard page (today's tasks + schedule)
- [ ] Task management pages (inbox, create, detail)
- [ ] Goals page
- [ ] Dexie.js IndexedDB setup (full schema from §17)
- [ ] Offline queue (ActionRouter pattern from §19.2)
- [ ] Service Worker (cache-first for app shell, network-first for API)
- [ ] WebLLM integration (download button in Settings, offline generation)
- [ ] Sync service (queue flush on reconnect)
- [ ] Offline banner UI
